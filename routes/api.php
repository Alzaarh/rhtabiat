...
Route::namespace ('User')->group(function () {
	...
	Route::apiResource('addresses', 'AddressController')->middleware('auth:user');
	...
	Route::put("users/orders/{order}/reorder", "OrderController@reorder")->middleware("auth:user");
}
);
...
Route::post('orders/verify', function () {
	redirect(url(env('PAYMENT_REDIRECT'), request()->input()));
});